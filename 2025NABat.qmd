---
title: "North American Bat Monitoring Program in Blackwater Mine"
...
author: 
  ...
  - name: "Brooke Daly"
    affiliation: "Manual Verifier"
editor: visual
bibliography: references.bib
nocite: '@*'
---

![](Images/73082-NW.jpg){style="float:left`;" fig-alt="Site deployment 73082-NW" fig-align="center"}
...
df <- read.csv("Data/GUANO_Complete_Metadata_20251202_132830.csv")
sites <- read.csv("Data/BlackWaterMine_Survey123.csv")

#make sure that the columns are the proper classes
sites$Date.Recording.Started <- dmy(format(mdy_hm(sites$Date.Recording.Started), "%d-%m-%Y"))
sites$Date.Recording.Ended <- dmy(format(mdy_hm(sites$Date.Recording.Ended), "%d-%m-%Y"))
...
#| include: true
#| fig-align: center
#| fig-cap: "Acoustic monitoring locations for bat surveys carried out Blackwater Mine in 2025."
...
All recordings with their associated tags have been uploaded to Wildtrax to the project named [Muskeseya Ispikaa'pawew IPCA - Bat Community - Presence/Absence Monitoring - NABat Data - 2025](https://portal.wildtrax.ca/aru/3856). Because recording names prefixes were saved as the recorder ID, we re-named the file names to match the location names([@tbl-rename]).

```{r}
#| label: tbl-rename
#| tbl-cap: "Updated location names used for Wildtrax uploads"
#| echo: false
#| warning: false
#| message: false

colnames(rename) <- c("NABat GRTS ID","Location",
                           "Location Identifier in Survey123",
                            "Location Name on Wildtrax",
                            "Original Recording Prefix")
rename$`NABat GRTS ID` <- as.character(rename$`NABat GRTS ID`)
...
if (knitr::is_latex_output()) {
  ...
  rename %>%
    ...
    align(j = 2:ncol(rename), align = "center") %>%
    theme_zebra() %>%
    width(width = 6, j = 1:5) %>%
    autofit()
}
